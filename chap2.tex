...
\underline{Idee: } Green: Addire harmonische Funktion, um unbekannten Term zu kompensieren.
...
$$
  u(x) = \frac{2 x_n}_{n \omega(n)} \int_{\partial \R_+^n} \frac{g(y){x - y}^n dy
$$
hat.

\subsection{Satz}

Sei $g \ion C(\R^{n  - 1}) \cap L^\infty(\R^{n - 1}$ und $u$ sei definiert wie oben. Dann:
\begin{enumerate}[i)]
